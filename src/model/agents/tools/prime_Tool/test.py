...
def save_excel() -> bool:
    """
    从PRIME输出文本文件读取数据并动态解析列，保存到Excel
    
    Args:
        output_file: 输入文本文件路径（.txt）
        output_dir: 输出目录路径 
    ...
    """
    output_file="/mnt/softwares/PRIME/test/out2.txt"
    output_dir="/mnt/softwares/PRIME/test"
    output_filename="out3.xlsx"
    ...
    try:
        ...
        logger.error(f"输入文件不存在: {output_file}")
        return False
